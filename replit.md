...
- **Modular Strategy System (Forex Bot)**: Plug-and-play strategy architecture (`strategies/`) with a `base_strategy.py` interface and `strategy_loader.py`. Includes a Multi-TP System for phased profit taking. Individual ATR multipliers for TP1/TP2/TP3/SL are configurable via `forex_config` table (`atr_tp1_multiplier`, `atr_tp2_multiplier`, `atr_tp3_multiplier`, `atr_sl_multiplier`).
- **Centralized Pip Calculator (Jan 2026)**: All pip calculations use `core/pip_calculator.py` with `PIP_VALUE=0.10` and `PIPS_MULTIPLIER=10` for XAU/USD. This is the single source of truth - no inline `* 100` calculations. Fix-forward approach: historical data before Jan 2026 uses old pip values (10x inflated).
...
- **Telegram Send Infrastructure (Dec 2025)**: Production-grade message sending via `core/telegram_sender.py`. Features: 60s TTL cache for bot connections (thread-safe), short-lived `telegram.Bot` instances per send (no cached bot objects), explicit cache invalidation when credentials saved/deleted in Connections UI, fail-fast behavior (no fallback bot - crisp errors when credentials missing), masked token logging (`****abcd`). All Telegram sends MUST go through `send_to_channel()`, `send_message()`, or `send_message_sync()` - no direct telegram.Bot instantiation allowed. The `send_message_sync()` function provides a synchronous wrapper using `asyncio.run()` for non-async contexts (journeys scheduler/engine). Endpoint `/api/connections/validate-saved` provides dry-run credential validation via Telegram API.
- **Telethon User Client (Feb 2026)**: Telegram User API integration via Telethon for Journey messaging without bot labels. Module at `integrations/telegram/` with: `user_client.py` (TelethonUserClient singleton per tenant, background asyncio event loop, per-account rate limiting 20/min 200/hr 500/day, FloodWait handling, auto-reconnect), `user_session.py` (DO Spaces session persistence), `user_auth.py` (3-step auth: send code -> verify -> optional 2FA), `user_handlers.py` (API endpoints at `/api/telethon/*`), `user_listener.py` (inbound message routing to journey sessions). Journey engine tries Telethon first, falls back to Bot API. Admin Connections tab includes User Account card with auth modal and status display. Environment variables: `TELEGRAM_API_ID`, `TELEGRAM_API_HASH`, `TELEGRAM_PHONE` for entrylab tenant.
...
**Database Availability Pattern**: All repository functions use `_require_db_pool()` guard that raises `DatabaseUnavailableError` when `db.db_pool` is None. Handlers catch this exception and return consistent 503 responses with `{'error': 'Database not available'}`. Repository functions also raise `DatabaseOperationError` on query failures instead of silently returning empty data. This ensures:
- Fail-fast behavior when database is unavailable (503)
- Fail-fast behavior on query execution errors (500)
- Consistent error responses across all API endpoints
- No silent failures or fallback data

**Tenant Isolation Pattern (Dec 2025)**: Handlers reject requests without valid tenant context with 403 "No tenant context available". No implicit fallback to 'entrylab' tenant occurs - if tenant resolution fails, requests are denied. Admin users correctly get 'entrylab' via explicit admin email whitelist check, not by fallback
