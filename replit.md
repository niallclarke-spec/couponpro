...
- **Cross Promo Automation**: Automated system for cross-promoting VIP signals in FREE channels, including scheduled jobs, Alpha Vantage gold news integration, atomic job claiming, and an automated morning sequence and end-of-day pip brag. TP-triggered cross-promotions are also handled. Daily morning post (06:00 UTC) and EoD recap (16:00 UTC) both route through the Hype Chat Bot's Markus SYSTEM_PROMPT for voice consistency. Morning Macro injects live XAU/USD spot from metals-api as ground truth (`domains/crosspromo/macro_data.py`). EoD recap is gated on `net_pips_today > 0` — silently skipped on red/flat days.
- **Hype Chat Bot**: AI-powered motivational messaging for FREE channels using OpenAI (gpt-4o-mini) with a fixed Markus "quiet certainty" SYSTEM_PROMPT and auto-injected pip performance context. Single source of truth for the FREE-channel voice across TP1 Win flow, Morning Macro, and EoD Recap. Admin UI supports prompt and flow CRUD, a step builder, preview, and analytics. Features a flexible step sequencer for flow orchestration, signal context injection, per-flow daily deduplication, and flow chaining.
- **Markus Signal Pause**: Cooperative gate that prevents a fresh VIP entry alert (and its TP-forward sequence) from racing with an in-flight Markus AI message on the FREE channel. New `markus_signal_pause` table (PK = tenant_id, with `expires_at` TTL safety net). Repo helpers `set_signal_pause` / `clear_signal_pause` / `get_active_signal_pause` in `domains/crosspromo/repo.py`. Service-level `_markus_signal_pause(tenant_id, reason, ttl)` context manager in `domains/crosspromo/service.py` wraps all 3 Markus surfaces in `send_job`: `morning_news` (TTL 120s), `eod_pip_brag` (TTL 120s, acquired AFTER the green-gate so red days don't waste a window), and each `hype_message` arc step (TTL 90s). `scheduler/generator.py:run_signal_check()` consults `get_active_signal_pause` right after the trading-hours check and skips the scan with a `⏸️` log line when a pause is active. Owner-scoped clear (UUID per acquire) ensures a stale finally-block can't drop a newer pause; the DB TTL guarantees signal generation auto-resumes even if a Markus job crashes. Gate failures fail-open (signal generation continues).
- **AI Voice Unification (Markus)**: Single Markus voice enforced across the FREE-channel TP1 path and VIP-channel mid-trade updates. The legacy `generate_forward_promo_message` / `_fallback_promo_message` / `generate_tp3_hype_message` rogue prompts were deleted from `domains/crosspromo/service.py`; TP forward jobs now rely only on the forwarded VIP signal/TP-hit message plus the Markus 3-arc fired via `trigger_flow_from_cta`. `MilestoneTracker._generate_40_percent_message` and the three prompts in `bots/core/ai_guidance.py` were rewritten to a Markus-aligned VIP-desk voice (no emojis, no hype words, no shame frames). `domains/hypechat/service.py` now ships a programmatic `_validate_message` post-generation guardrail with regex-pattern banned families and label-fusion detection; rejected drafts trigger a regenerate-with-corrective-system-message loop (max 2 retries) before falling back to a curated static line. `build_context` wraps each pip number in an unmissable "PIP NUMBER #N" block with explicit TIME WINDOW + "DO NOT call this 'today'" framing to prevent today/streak label-fusion. The deprecated `send_test_forward_promo` admin endpoint redirects to `send_test_markus_tp1_realistic`.
