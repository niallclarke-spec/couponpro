...
- **Journeys System**: Event-triggered, multi-step conversational flows for Telegram bots via deep links or direct messages. It supports various step types (message, question, delay, wait_for_reply), a background scheduler (10s polling interval), and step analytics including link click tracking. Telethon user clients handle private messages. `{first_name}` placeholder works in any message/question step text, replaced with the user's Telegram first name (stored as `_first_name` in session answers JSON at creation time). Configurable start delay (`start_delay_seconds` column on journeys, 0-180 min, user-selectable in seconds/minutes): 0 = step 1 executes immediately on trigger, >0 = step 1 scheduled via scheduler after delay. Single code path for all delays (no thread/welcome complexity). Inline execution for step delays ≤15s (exact timing via sleep), deferred to scheduler for >15s. Production reliability features include: per-user async locking (LRU-evicting, max 1000), message deduplication via `journey_inbound_dedupe` table, session auto-heal (broken sessions marked and fall through to DM triggers), inactivity auto-complete (configurable `inactivity_timeout_days` per journey, enforced in both listener and scheduler), stale job guard (scheduler validates session status + step match before sending), journey priority (`priority_int` column, highest priority wins on trigger match), journey locking (`is_locked` prevents edits, duplicate endpoint clones journey+steps+triggers), and await-reply keyword branching (`branch_keyword`, `branch_true_step_id`, `branch_false_step_id` on journey_steps).
...
- **Conversion Tracking System**: Full customer lifecycle tracking using unique Telegram invite links. Landing page captures email + UTMs → `POST /api/telegram/grant-access` with `amountPaid=0` → generates single-use FREE channel invite link (72h expiry, `member_limit=1`) → user joins FREE channel → webhook matches invite link to email, bridging email to Telegram identity → user pays for VIP → Stripe webhook detects free→VIP conversion, sets `is_converted=TRUE`, calculates `conversion_days`. Fallback: configurable public link (`free_channel_link` on `tenant_bot_connections`), retry logic (3 attempts with 1s delay), organic join tracking for users without invite links. UTM auto-append on Hype Bot CTA URLs (`utm_source=telegram&utm_medium=free_channel&utm_campaign=hype_bot`). Admin dashboard at `#forex/conversions` shows funnel (leads → joined → VIP), conversion metrics, UTM attribution table, and recent activity with time period filters.
- **Connections System**: Tenant-specific bot configuration for Signal Bot and User Account (Telethon), storing bot tokens, channel IDs, webhook secrets, and `free_channel_link` (public fallback). This is the single source of truth for bot credentials AND webhook URLs. On server startup, `core/bootstrap.py` re-registers all webhooks from the `tenant_bot_connections` table with `allowed_updates: ['message', 'chat_member']`. The generic webhook handler (`/api/bot-webhook/<secret>`) handles journey triggers, journey replies, AND channel join tracking (chat_member events for invite link matching). Message Bot is removed from the UI. Connections page uses iOS-style single-column expandable accordion layout.
...
- **Telethon User Client**: Telegram User API integration for Journey messaging without bot labels, featuring a singleton client per tenant, rate limiting (soft 1200/hard 1500 daily, with get_level() for scheduler pacing), and session persistence.
- **Cross Promo Automation**: Automated system for cross-promoting VIP signals in FREE channels, including scheduled jobs, Alpha Vantage gold news integration, and atomic job claiming. Morning sequence posts three timed messages to FREE channel (morning news, weekly recap forward if available, VIP teaser) at configurable UTC times (default 06:42, 07:12, 07:51). Weekly recap message IDs are tracked so they can be forwarded from VIP to FREE. End-of-day pip brag at 20:00 UTC. Cross Promo settings are embedded within the Signal Bot connection card on the Connections page (no standalone page). **TP-triggered cross promo lifecycle**: TP1 → forward signal+TP1 to FREE, start 30-min countdown; TP2 → forward TP2, reset countdown; TP3 → forward TP3, finish immediately. If countdown expires or SL hits after TP1, cross promo finishes. Finish triggers hype bot. Old CTA job removed — hype bot CTA replaces it. Job dedupe uses DELETE (not UPDATE) to free dedupe slots for rescheduling.
- **Hype Chat Bot**: AI-powered motivational messaging for FREE channel. Uses OpenAI (gpt-4o-mini) with a fixed EntryLab system prompt and auto-injected pip performance context (today + 7-day). Admin creates named Prompts (custom instructions for message angle/style) and Flows (scheduling rules: message count, interval between messages, delay after cross-promo finish, active days with visual toggle buttons and server-side conflict validation). Triggers automatically after cross-promo finishes, once per day, with deduplication. Messages are scheduled as crosspromo_jobs with staggered run_at times. Admin UI at `#forex/hype-bot` with prompt CRUD, flow CRUD, preview (generate without sending), manual trigger, and per-flow analytics. Domain: `domains/hypechat/` (repo, service, handlers).
