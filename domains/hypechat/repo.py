...
def get_today_hype_count_for_flow(tenant_id: str, flow_id: str) -> int:
    db_pool = _get_db_pool()
    if not db_pool or not db_pool.connection_pool:
        return 0
    ...
    try:
        with db_pool.get_connection() as conn:
            ...
            cursor.execute("""
                SELECT COUNT(*)
                FROM hype_messages
                WHERE tenant_id = %s AND flow_id = %s AND sent_at >= CURRENT_DATE
            """, (tenant_id, flow_id))
            row = cursor.fetchone()
            return int(row[0]) if row and row[0] else 0
    except Exception as e:
        logger.exception(f"Error getting today hype count for flow: {e}")
        return 0
